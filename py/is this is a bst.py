...
def is_bst(node:Node, lower_lim=None, upper_lim=None):
    if lower_lim != None and node.value <= lower_lim and upper_lim != None and node.value >= upper_lim:
...
testit(lambda:is_bst(head0), False)
testit(lambda:is_bst(head1), False)
testit(lambda:is_bst(head2), False)
testit(lambda:is_bst(head3), True)
testit(lambda:is_bst(head4), False)